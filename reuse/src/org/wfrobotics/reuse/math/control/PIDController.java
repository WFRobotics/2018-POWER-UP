...
public final class PIDController
{
    ...
    public double update(double error)
    {
        ...
        if(preStageClamp) proportional = Util.clampToRange(proportional, m_MaxOutput);

        double derivative = 0;
        if(timeDiff > 0)
        {
            derivative = (error - lastError) / timeDiff * m_D;
            if(preStageClamp) derivative = Util.clampToRange(derivative, m_MaxOutput);
        }
        ...
        if (timeDiff <= MaxTimestampDiff)
        {
            ...
            if(preStageClamp) integral = Util.clampToRange(integral, m_MaxOutput);
        }
        else
        {
        ...
        clampedVal = Util.clampToRange(clampedVal, m_MaxOutput);
