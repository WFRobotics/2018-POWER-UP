package org.wfrobotics.reuse.subsystems.control;
 ...
 *
 * <p>Implements an adaptive pure pursuit controller. See:
 * https://www.ri.cmu.edu/pub_files/pub1/kelly_alonzo_1994_4/kelly_alonzo_1994_4 .pdf
 *
 * <p>Basically, we find a spot on the path we'd like to follow and calculate the arc necessary to make us land on that
...
public class PurePursuitController {
    ...
    public static class PursuitCommand {
        ...
        public PursuitCommand() {
        }

        public PursuitCommand(Twist2d delta, double cross_track_error, double max_velocity, double end_velocity,
     ...
     */
    public PursuitCommand update(Pose2d pose) {
        if (mReversed) {
        ...
        if (isFinished()) {
            // Stop.
            return new PursuitCommand(Twist2d.identity(), report.closest_point_distance, report.max_speed, 0.0,
