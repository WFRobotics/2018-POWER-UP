...
{
    private static final int kTimeoutMs = 100;  // Use zero during teleop, something longer in Subsystem constructors

    private TalonFactory() { /** No instances */ };

    /** Simple, <b>single threshold</b> current limiting. Start with this. */
    public static void configCurrentLimiting(TalonSRX t, int continuousAmps)
    {
        configCurrentLimiting(t, continuousAmps, 0, 0);
    }

    /** <b>Two threshold</b> current limiting. May have better performance during setpoint changes. Allows a brief, <i>desirable</i> spike in current. */
    ...
    public static List<TalonSRX> makeClosedLoopTalon(ClosedLoopConfig config)
    {
        ...
        for (MasterConfig m : config.masters)
        {
            ...
            configEncoder(t);
            t.setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 10, kTimeoutMs);  // Greater than robot update rate
    ...
    private static void configDefaults(BaseMotorController t)
    {
        ...
        t.configNeutralDeadband(0.4, 10);                      // Output promoted to zero below this threshold, same effect as software deadband but also affects closed loop
        ...
        if (t instanceof TalonSRX)
        {
            ...
            talon.setStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 10, kTimeoutMs);  // Keep decent limit switch state
        ...
        {
            t.setStatusFramePeriod(StatusFrame.Status_1_General, 10, kTimeoutMs);
